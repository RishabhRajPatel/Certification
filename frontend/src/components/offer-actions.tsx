...
import { issueOffer, sendOfferEmail } from "@/lib/actions/offers";
import type { Offer } from "@/lib/types";

const HISTORY_LABELS: Record<string, string> = {
  GENERATE_OFFER: "Offer letter created (draft)",
  ISSUE_OFFER: "Offer letter issued",
  SEND_OFFER_EMAIL: "Email sent",
  EMAIL_SEND_FAILED: "Email failed to send",
  DOWNLOAD_OFFER: "PDF downloaded",
  VOID_OFFER: "Voided",
};
...
export function OfferActions({ offer, autoSendEnabled }: { offer: Offer; autoSendEnabled: boolean }) {
  const router = useRouter();
  const [pending, startTransition] = useTransition();
  const [error, setError] = useState<string | undefined>();

  function handleIssue() {
    if (!confirm("Issue this offer letter?" + (autoSendEnabled ? " The email will be sent immediately." : ""))) return;
    setError(undefined);
    startTransition(async () => {
      const res = await issueOffer(offer.id);
        ...
        <CardContent className="space-y-4">
          ...
          {isDraft ? (
            <Button onClick={handleIssue} disabled={pending} className="w-full">
              ...
              {pending ? "Issuing…" : autoSendEnabled ? "Issue & Send Email" : "Issue Offer Letter"}
            </Button>
          ) : (
            !autoSendEnabled && !offer.emailStatus && (
          ...
          <CardContent className="space-y-3">
            {offer.emailStatus === "SENT" && (
              <div className="flex items-center gap-2 text-sm text-emerald-700">
                <CheckCircle2 className="h-4 w-4" /> Sent to {offer.emailRecipient}
                {offer.emailSentAt && <span className="text-slate-400">· {formatDateTime(offer.emailSentAt)}</span>}
              </div>
            )}
            {offer.emailStatus === "FAILED" && (
              <div className="space-y-1.5">
                ...
                {offer.emailError && <p className="text-xs text-slate-500">Reason: {offer.emailError}</p>}
              </div>
            )}
            {!offer.emailStatus && (
            ...
            <Button onClick={handleSendEmail} disabled={pending} variant="outline" className="w-full">
              {pending ? <Loader2 className="h-4 w-4 animate-spin" /> : offer.emailStatus === "FAILED" ? <RotateCcw className="h-4 w-4" /> : <Send className="h-4 w-4" />}
              {pending ? "Sending…" : offer.emailStatus === "FAILED" ? "Retry Email" : offer.emailStatus === "SENT" ? "Resend Email" : "Send Email"}
            </Button>
          </CardContent>
        </Card>
      )}
